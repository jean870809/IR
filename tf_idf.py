...
with open('stop_words.txt', encoding  = 'utf-8', newline='\n') as txtfile:
...
for i in range(3,4):
    ...
    for ind, file in enumerate(os.listdir(r'txt')):
        ...
        txt = file.read()
        strr = ''.join(strr.split()) #將歌名字串中的空格拿掉
        txt += " " + strr  #放歌名進去切 #空格拿掉
        temp_list = list()
        temp_list.append(txt)
        if i == 0:
            # sent_words = ws(temp_list)
            filename = "output_file_taiwan_no_stopwords_blankspace/ckiptagger_output.csv"
        elif i == 1:
            sent_words = [list(jieba.cut(txt))]
            filename = "output_file_taiwan_no_stopwords_blankspace/jieba_normal_output.csv"
        elif i == 2:
            sent_words = [list(jieba.cut(txt, cut_all=True))]
            filename = "output_file_taiwan_no_stopwords_blankspace/jieba_cut_all_output.csv"
        else:
            sent_words = [list(jieba.cut_for_search(txt))]
            filename = "output_file_taiwan_no_stopwords_blankspace/jieba_search_output.csv"

        sent_words[0].append(strr) #加入整個歌名 #去掉空格
    ...
    vectorizer = TfidfVectorizer(sublinear_tf=False, token_pattern="(?u)\\b\w+\\b", smooth_idf=True, norm='l2')
